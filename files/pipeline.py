...
def run(
    ...
    start_str = start_date or (
        today - timedelta(days=DEFAULT_LOOKBACK_DAYS)).strftime("%Y-%m-%d")
    run_str = today.strftime("%Y-%m-%d")

    # --- Account mặc định ---
    if account_ids:
        active_accounts = {a: AD_ACCOUNTS.get(
            a, "") for a in account_ids if a in AD_ACCOUNTS}
    else:
    ...
    gcs_thumbnail_map = {}
    try:
        gcs_thumbnail_map = upload_thumbnails_to_gcs_batch(
            ad_details, GCS_BUCKET_NAME)
        logger.info(
            f"[B2.5] Hoàn tất đồng bộ {len(gcs_thumbnail_map)} ảnh lên GCS.")
    except Exception as gcs_err:
        logger.error(
            f"[B2.5] Bỏ qua lỗi đồng bộ ảnh GCS (luồng chính vẫn tiếp tục): {gcs_err}")
    ...
    for account_id in active_accounts.keys():
        try:
            ...
            raw_age_gender = fetch_demographics_insights(
                account_id, start_str, end_str, "age,gender")
            demo_rows.extend(transform_demographics(
                raw_age_gender, active_accounts, "age_gender", run_str))

            # Lấy và transform Region
            raw_region = fetch_demographics_insights(
                account_id, start_str, end_str, "region")
            demo_rows.extend(transform_demographics(
                raw_region, active_accounts, "region", run_str))

        except Exception as demo_err:
            logger.error(
                f"[B5] Lỗi lấy demographics cho {account_id}: {demo_err}")

    inserted_demo = 0
    if demo_rows:
        logger.info(
            f"[B5] Insert {len(demo_rows)} dòng demographics vào BigQuery...")
        try:
            inserted_demo = upsert_demographics_rows(
                demo_rows, start_str, end_str, list(active_accounts.keys()))
        except Exception as bq_demo_err:
            logger.error(
                f"[B5] Lỗi insert demographics vào BigQuery: {bq_demo_err}")

    logger.info(
        f"[Pipeline] HOÀN TẤT — đã insert {inserted} dòng chính và {inserted_demo} dòng nhân khẩu học.")
    logger.info("=" * 60)
