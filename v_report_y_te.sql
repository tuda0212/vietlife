...
-- B1: Gom nhiều ad cùng bài post lại thành từng dòng theo ngày cho chuyên khoa Y tế
post_metrics AS (
  SELECT
    start_date,
    end_date,
  ...
  WHERE spend > 0 
    AND specialty_code IN ('CXK', 'TK', 'CS', 'TH')
  GROUP BY 1,2,3,4,5,6,7
),

-- B2: Gom CRM theo ad_id và lead_date cho nhóm Y tế
crm_per_ad AS (
  SELECT
    ad_id,
    lead_date,
  ...
  WHERE lead_date IS NOT NULL 
    AND report_group = 'Y tế'
  GROUP BY ad_id, lead_date
),

-- B3: Ghép CRM vào từng post theo ngày
crm_per_post AS (
  SELECT
    p.start_date,
    p.end_date,
...
-- Portion 2: Gom CRM chưa được map (Tự nhiên hoặc Ad không chạy trong ngày đó) cho nhóm Y tế
