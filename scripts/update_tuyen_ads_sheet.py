...
logger = logging.getLogger("update_tuyen_ads_sheet")

def main():
    project_id = "gen-lang-client-0738410622"
    dataset_id = "marketing_data"
    spreadsheet_id = "1G1EryZY1pN81L2yW3p7IrSgTwNa-TmXKXnLIiEGVO5c"
    target_gid = 2029375259
    
    # 1. Truy vấn BigQuery lấy các ad của bác sĩ Tuyên
    logger.info("Đang truy vấn BigQuery để lấy danh sách bài quảng cáo của BS Tuyên...")
    ...
    for row in results:
        # Nếu có thumbnail, dùng công thức =IMAGE để hiển thị ảnh trực quan
        thumb_formula = f'=IMAGE("{row.thumbnail_url}")' if row.thumbnail_url else ""
        content = row.content or ""
        # Cắt bớt content nếu quá dài (Google Sheets giới hạn 50,000 ký tự trong một ô, nhưng để dễ đọc nên để nguyên hoặc cắt nếu quá dài)
        # Ở đây ta giữ nguyên content
